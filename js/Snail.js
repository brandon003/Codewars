...
snail = function (array) {
	...
	for (let i = 0, move = 1; i < 2 * n - 1; i++) {
		...
		switch (move) {
			...
			case 1: // Move 1: add the whole top array
				result = result.concat(array.shift())
				console.log(result)
				move++
				break;

			case 2: // Move 2: add right side of matrix
				for (let j = 0; j < array.length; j++) {
					result = result.concat(array[j].pop())
				}
				console.log(result)
				move++
				break;

			case 3: // Move 3: add the whole bottom array in reverse
				result = result.concat(array.pop().reverse())
				console.log(result)
				move++
				break;

			case 4:// Move 4: add left side of matrix
				let tempArray = [];
				for (let j = 0; j < array.length; j++) {
					tempArray = tempArray.concat(array[j].shift())
				}
				result = result.concat(tempArray.reverse())
				console.log(result)
				move = 1
				break;
		}
